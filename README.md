...
- 仓库自带的 `docker-compose.yml` 已把 `AETHER_GATEWAY_AUTO_PREPARE_DATABASE` 设为默认开启，因此 `docker compose up -d` 会在监听端口前自动执行挂起的 migration 和 backfill。
...
当前不应该再依赖旧的 `alembic downgrade` 路线。空库首次启动会自动初始化；如果开启 `AETHER_GATEWAY_AUTO_PREPARE_DATABASE=true`（仓库自带 `docker-compose.yml` 默认如此），常规服务启动也会自动应用挂起的 migration/backfill。无论是否自动执行，只要本次发布带来了不可逆的数据结构变化，没有备份就不能保证安全回滚。因此升级前强烈建议先备份 `Postgres`。
