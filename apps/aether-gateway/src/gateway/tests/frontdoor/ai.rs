use crate::gateway::tests::{
    ...
    start_server, to_bytes, AppState, Arc, Body, Json, Mutex, Request, Router, StatusCode,
...
use super::{
    ...
    DEVELOPMENT_ENCRYPTION_KEY, InMemoryAuthApiKeySnapshotRepository,
...
async fn gateway_handles_gemini_operation_detail_without_hitting_fallback_probe() {
    ...
    let gateway = build_router_with_state(
        AppState::new()
            .expect("gateway should build")
            .with_data_state_for_tests(
                crate::gateway::gateway_data::GatewayDataState::with_auth_and_video_task_repository_for_tests(
                    auth_repository,
                    repository,
                ),
            ),
