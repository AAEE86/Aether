...
def _process_message_content(content: Any) -> tuple[str, list[dict[str, Any]], list[dict[str, Any]]]:
...
def convert_claude_messages_to_conversation_state(
    ...
    *,
    ...
    if not model_id:
        raise ValueError(f"kiro: unsupported model: {model}")
    ...
    # Build history from messages except last (assume last is user input).
    ...
    existing_tool_names = {str(t.get("toolSpecification", {}).get("name", "")).lower() for t in tools}
