...
The output format intentionally uses only `data: {...}\n\n` lines. Event names are
encoded in the JSON `type` field, matching how Aether parses Claude streams.
"""
...
class _KiroStreamState:
    ...
    def _ensure_text_block_open(self) -> list[dict[str, Any]]:
        if self.text_block_index is not None:
            if self.text_block_index in self.open_blocks and self.open_blocks[self.text_block_index] == 'text':
                return []
        ...
        while True:
            ...
            if self.in_thinking_block:
                ...
                if end_pos is not None:
                    ...
                    # Close thinking block: send empty delta then stop.
                    if self.thinking_block_index is not None:
        ...
        try:
            ...
            try:
                ...
                if isinstance(error_json, dict):
                    # 提取上游错误信息
                    upstream_msg = error_json.get('message') or error_json.get('error', {}).get('message')
                    if upstream_msg:
            ...
            if mtype == 'event':
                ...
                if etype == 'toolUseEvent':
                    if isinstance(payload, dict):
                        ...
                        input_json = str(input_json or '') if input_json is not None else ''
                ...
                yield _sse_data_bytes(
                    {
                        ...
                        'error': {
                            ...
                            'message': f'{ex_type}: {payload_text[:200]}',
                        },
                    }
                )
                continue
                ...
                yield _sse_data_bytes(
                    {
                        ...
                        'error': {
                            'type': 'upstream_error',
                            'message': f'{err_code}: {payload_text[:200]}',
                        },
                    }
                )
                continue
...
__all__ = ["rewrite_eventstream_to_sse"]
