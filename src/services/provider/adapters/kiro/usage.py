...
async def fetch_kiro_usage_limits(
    ...
    # 构建请求
    region = (updated_cfg.region if updated_cfg else cfg.region) or "us-east-1"
    ...
    if response.status_code != 200:
        body_text = (response.text or "").strip()[:200]
        error_msg = {
        ...
        raise RuntimeError(f"{error_msg}: HTTP {response.status_code} {body_text}")
